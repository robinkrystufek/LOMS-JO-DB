(() => {
  ...
  const input = document.getElementById('is-revision-of-id');
  const statusDiv = document.getElementById('jo-rev-status');
  if (!input || !statusDiv) return;

  function updateStatus() {
    const val = input.value?.trim();
    if (val && /^\d+$/.test(val)) {
      statusDiv.textContent = `Revision of record ${val}`;
    } else {
